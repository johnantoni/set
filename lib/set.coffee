...
set = (root, obj) ->
  ...
  obj.__set = {} if obj.__set is undefined

  # Shortcut to flag: are we running on the server?
  isRunningOnServer = obj.__set.server

  # Render static option.
  shouldRenderStatic = isRunningOnServer and obj.__set.renderStatic
  ...
  qif = qdef.qif or "data-set-if"
  qrepeat = qdef.qrepeat or "data-set-repeat"
  qattr = qdef.qattr or "data-set-attribute"
  qtext = qdef.qtext or "data-set-text"
  qdup = qdef.qdup or "data-set-dummy"

  # Output formatter.
  format = qdef.format
  qdef = qdef.qdef or "data-set-alias"
  ...
  loop
    ...
    # e.g., <section data-set-alias='feeds main.sidebar.feeds'>
    ...
    # e.g., <img data-set-if='item.unread'> <img data-set-if='item.count isGreaterThan 1'>
    ...
    # of the loop.
    # e.g., <div data-set-repeat='item feeds.items'>
    ...
    if attr
      ...
      throw new Error('missing second attribute (collection name) in data-set-repeat: ' + node.outerHTML + '. The correct syntax is data-set-repeat="value collection". e.g., to iterate over an array called people: data-set-repeat="person people". And then, from within the loop, you can access properties of the person object (e.g., person.name, person.age, etc.)') unless attr2[1]
      ...
      if objList.length > 1
        ...
        # html = "<div data-set-alias=' + [1,2,3].join('><div data-set-alias=') + '>'
    ...
    # e.g., <div data-set-attribute='value item.text; disabled item.disabled'>
    #

    # Catch empty data-set-attribute attributes to help in debugging.
    ...
    if attr
      ...
      if attr == ''
        throw new Error('empty data-set-attribute definition on element: ' + node.outerHTML)
        ...
        if altAttr[name]
          switch name
            when "innerHTML" #should use "qtext"
              throw new Error('setting the innerHTML attribute via data-set-atttribute is not supported. Please use data-set-text to set the text in ' + node.outerHTML)
      ...
      # Try and catch any empty data-set-attribute attributes to help the user debug.
      if node.hasAttribute
        if node.hasAttribute(qattr)
          throw new Error('empty data-set-attribute definition on element: ' + node.outerHTML)

    # Sets the innerHTML on the node.
    # e.g., <div data-set-text='html item.description'>
